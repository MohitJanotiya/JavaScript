...
if (typingText) {
    ...
    function typeWriter() {
        if (i < prefix.length) {
            ...
            i++;
            setTimeout(typeWriter, 100);
        } else if (i <= prefix.length + highlightText.length) {
            const span = typingText.querySelector('.highlight');
            const charIndex = i - prefix.length - 1;
            if (charIndex < highlightText.length) {
                span.textContent += highlightText.charAt(charIndex);
                i++;
                setTimeout(typeWriter, 100);
            }
        }
